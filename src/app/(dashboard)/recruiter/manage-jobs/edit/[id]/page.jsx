...
import { Briefcase, MapPin, UserCheck, FileText, Calendar, Plus, X, DollarSign, Loader2, ArrowLeft } from "lucide-react";
import { useEffect, useState } from "react";
import { useRouter, useParams } from "next/navigation";
import { getSingleJobAction, updateJob } from "@/lib/actions/jobs";
...
function EditJob() {
    ...
    // Fetch job data and prepopulate the form fields
    useEffect(() => {
        const fetchAndPopulateData = async () => {
            if (!id) return;
            try {
                const response = await getSingleJobAction(id);
                ...
                Swal.fire({
                    icon: "error",
                    title: "Fetch Error",
                    text: "Could not retrieve job details. Returning to dashboard.",
                    background: "#0A0A0A",
                    color: "#ffffff",
    ...
    const onSubmit = async (data) => {
        const confirmation = await Swal.fire({
            title: "Are you sure?",
            text: "Do you want to save the changes made to this job listing?",
            icon: "warning",
        ...
        try {
            ...
            if (res?.success) {
                await Swal.fire({
                    icon: "success",
                    title: "Job Updated 🚀",
                    text: res.message || "Your job was updated successfully.",
                    background: "#0A0A0A",
                    color: "#ffffff",
                    confirmButtonColor: "#10b981",
                    timer: 2200,
    ...
    if (pageLoading) {
        return (
            <div className="w-full flex flex-col justify-center items-center py-32 text-[#1c4a36] dark:text-[#e4f5ee]">
                <Loader2 className="animate-spin text-[#1c4a36] dark:text-[#e4f5ee] mb-2" size={32} />
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-6 pb-12 p-4 md:p-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-[#1c4a36] dark:text-[#e4f5ee]">Edit Job Listing</h1>
                    <p className="text-sm text-slate-500 dark:text-[#e4f5ee]/60 mt-1">Update the information below and save your changes.</p>
                </div>
                <button
                    type="button"
                    onClick={() => router.back()}
                    className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold uppercase tracking-wider rounded-xl border border-[#e4f5ee] dark:border-[#173f2e] hover:bg-slate-50 dark:hover:bg-[#173f2e] transition-colors cursor-pointer text-[#1c4a36] dark:text-[#e4f5ee]"
                >
                    <ArrowLeft size={14} />
                    <span>Back</span>
                </button>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="flex justify-end gap-4 pt-2">
                    <button type="button" onClick={() => router.push("/dashboard/my-jobs")} className="px-6 py-2.5 rounded-xl text-sm font-medium border border-slate-200 text-slate-600 hover:bg-slate-50 dark:border-[#173f2e] dark:text-[#e4f5ee]/70 dark:hover:bg-[#173f2e]">Cancel</button>
                    <button type="submit" disabled={isSubmitting} className="px-8 py-2.5 rounded-xl text-sm font-semibold shadow-md bg-[#1c4a36] text-white hover:bg-[#163b2b] disabled:opacity-50 dark:bg-[#e4f5ee] dark:text-[#1c4a36] dark:hover:bg-white">{isSubmitting ? "Updating..." : "Save Changes"}</button>
                </div>
            </form>
        </div>
    );
